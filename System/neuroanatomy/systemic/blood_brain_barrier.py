...
class ToxinDetector(ast.NodeVisitor):
    def __init__(self):
        self.is_toxic = False
        self.threat_reason = ""
        # REFACTOR: Moving from a blunt blacklist to an explicit whitelist of safe standard libraries
    ...
    def _is_module_allowed(self, module_name: str) -> bool:
        ...
        return (
            base_module in self.allowed_modules or module_name in self.allowed_modules
        )
    ...
    def visit_Import(self, node):
        for alias in node.names:
            if not self._is_module_allowed(alias.name):
                self.is_toxic = True
                self.threat_reason = f"AST MEMBRANE BLOCK: Non-whitelisted import detected '{alias.name}'."
        self.generic_visit(node)

    def visit_ImportFrom(self, node):
        if node.module and not self._is_module_allowed(node.module):
            self.is_toxic = True
            self.threat_reason = f"AST MEMBRANE BLOCK: Non-whitelisted from-import detected '{node.module}'."
        self.generic_visit(node)
